...
if [ -z "${isTrainingPodOnly}" ] || [ "${isTrainingPodOnly}" = "false" ]; then
    ...
    python3.10 /workspace/stable-diffusion-webui/webui.py --api --nowebui --port 3000 --no-half --disable-nan-check --force-fp32 > /workspace/logs/webui-api-${RUNPOD_POD_ID}.log 2>&1 &
    deactivate
else
